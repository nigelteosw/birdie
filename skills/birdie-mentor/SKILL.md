---
name: birdie-mentor
description: TRIGGER — capture proactively, without being asked, the moment a more senior person corrects, edits, or improves a more junior person's code, draft, or document anywhere in the conversation (a PR review, an inline fix to someone else's work, "here's how I'd rewrite what Lauren sent" — any before/after where one named person improved another's work). Also use for first-time setup when Birdie says it isn't configured, for listing/reviewing/promoting pending lessons, and for answering "how would a senior handle X" or "what's a junior struggling with" questions.
---

# Birdie Mentor

Birdie stores reviewed lessons from before/after edits between a senior and a junior. Use the `birdie` MCP tools directly.
...
Don't wait to be asked and don't wait for the user to say "capture this." The moment you notice a senior correcting or improving a junior's code, draft, or document — in this conversation, in a diff you're shown, in a PR you're reviewing — call `capture_trace`:

- `before_text` / `after_text`: the original and the corrected content, verbatim (not summarized or truncated).
- `submitted_by` / `submitted_by_role`: infer from context who is acting as senior and who as junior (e.g. "Nigel reviewing Lauren's draft" → `submitted_by: "Nigel"`, `submitted_by_role: "senior"`). If the roles genuinely aren't clear from context, ask once, briefly — then remember the answer for the rest of the conversation instead of asking again.
- `junior_name` / `senior_name`: fill in whenever known, so lessons can later be filtered per person.
...
4. Answer juniors with `ask-senior-approach`.
5. Answer seniors with `ask-junior-struggles`.
...
Only promoted lessons should be used when answering. If Birdie returns no reviewed examples, say so plainly.
